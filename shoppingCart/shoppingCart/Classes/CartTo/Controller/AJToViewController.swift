...
class AJToViewController: UIViewController,CAAnimationDelegate {
    ...
    override func viewDidLoad() {
        ...
        for i in 0..<10 {
            ...
            dict1["iconName"] = "goodicon_\(i)" as AnyObject
            dict1["title"] = "\(i)commodity" as AnyObject
            dict1["desc"] = "这是第\(i)个商品" as AnyObject
            dict1["newPrice"] = "20\(i)" as AnyObject
            dict1["oldPrice"] = "30\(i)" as AnyObject
            
            let model:AJGoodModel =  AJGoodModel(dict: dict1)
            //将字典转模型并添加到数组中
...
//        btn.setImage(UIImage(named: "button_cart"), for: UIControlState())
        btn.addTarget(self, action:Selector(("cartClick")) , for: UIControlEvents.touchUpInside)
//        btn.addTarget(self, action: Selector("cartClick"), for: UIControlEvents.touchUpInside)
        btn.sizeToFit()
